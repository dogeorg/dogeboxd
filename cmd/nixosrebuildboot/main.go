...
// Instead, run the wrapper that should be setuid @ /run/wrappers/bin/nixosrebuildboot
...
func main() {
	...
	cmd := exec.Command("sudo", "nixos-rebuild", "boot")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr

	if err := cmd.Run(); err != nil {
		fmt.Fprintln(os.Stderr, "Error executing nixos-rebuild boot:", err)
		os.Exit(1)
	}
}
